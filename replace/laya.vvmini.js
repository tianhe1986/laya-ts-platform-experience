...
var MiniFileMgr$4=(function(){
	...
	MiniFileMgr.downFiles=function(fileUrl,encoding,callBack,readyUrl,isSaveFile,fileType,isAutoClear){
		...
		var downloadTask=MiniFileMgr.wxdown({url:fileUrl,success:function (data){
				...
				if (data.statusCode===200) {
					//必须先拷贝到本地再处理, 为了不影响原有流程, 拷贝到cache目录中,多做一次处理
					var temp = fileUrl.split("/");
					var cacheFileName = "internal://cache/" + temp[temp.length-1];
					MiniFileMgr.fs.copyFile({srcUri:data.tempFilePath,dstUri:cacheFileName,success:function (data2){
							MiniFileMgr.readFile(cacheFileName,encoding,callBack,readyUrl,isSaveFile,fileType,isAutoClear);
						}, fail:function (data3){
							callBack !=null && callBack.runWith([1,data3]);
						}
					});
				}
				else
	...
	MiniFileMgr.downLoadFile=function(fileUrl,fileType,callBack,encoding){
		(fileType===void 0)&& (fileType="");
		(encoding===void 0)&& (encoding="utf8");
		if(VVMiniAdapter.window.navigator.userAgent.indexOf('vivo')<0){
	...
	MiniFileMgr.writeFilesList=function(fileurlkey,filesListStr,isAdd){
		...
		MiniFileMgr.fs.writeFile({uri:listFilesPath,encoding:'utf8',text:filesListStr,success:function (data){
				},fail:function (data){
		}});
	...
	MiniFileMgr.readSync=function(filePath,encoding,callBack,readyUrl){
		...
		try{
			...
			if(typeof filesListStr === 'string'){
				filesListStr={"text" : JSON.stringify({})};
			}
			callBack !=null && callBack.runWith([0,{data:filesListStr.text}]);
		}
		catch(error){
			callBack !=null && callBack.runWith([1]);
		}
...
var VVMiniAdapter=(function(){
	...
	VVMiniAdapter.init=function(isPosMsg,isSon){
		...
		if(!VVMiniAdapter.window.hasOwnProperty("qg"))
			return;
		if(VVMiniAdapter.window.navigator.userAgent.indexOf('vivo')<0)return;
	...
	*/
	__proto.load=function(url,type,cache,group,ignoreCache){
		...
		if ((MiniLoader._fileTypeArr.indexOf(urlType)!=-1)){
			...
			var remoteUrl = URL.formatURL(url);
			if (!MiniFileMgr$4.getFileInfo(remoteUrl)){
				if (MiniFileMgr$4.isLocalNativeFile(url)){
					if (VVMiniAdapter.subNativeFiles && VVMiniAdapter.subNativeheads.length==0){
						for (var key in VVMiniAdapter.subNativeFiles){
							var tempArr=VVMiniAdapter.subNativeFiles[key];
				...
				if (url.indexOf("http://")!=-1 || url.indexOf("https://")!=-1){
					...
					MiniFileMgr$4.downFiles(url,encoding,new Handler(MiniLoader,MiniLoader.onReadNativeCallBack,[encoding,url,type,cache,group,ignoreCache,thisLoader]),url);
					}else {
				...
				var fileObj=MiniFileMgr$4.getFileInfo(remoteUrl);
				var fileNativeUrl=MiniFileMgr$4.getFileNativePath(fileObj.md5);
				fileObj.encoding=fileObj.encoding==null ? "ascii" :fileObj.encoding;
				MiniFileMgr$4.readFile(fileNativeUrl,fileObj.encoding,new Handler(MiniLoader,MiniLoader.onReadNativeCallBack,[encoding,url,type,cache,group,ignoreCache,thisLoader]),url);
			}
		}
	}
